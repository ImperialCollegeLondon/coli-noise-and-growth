...
%% ref parameters
def_pars = pombe_give_ref_pars () ;

%% compute demands
demands = pombe_compute_demands () ;
    ...
    for i_response = 1:length(response_types)
        ...
        for i_mu = 1:length(mu_vec)
            ...
            responses.(demand_types{i_demand}).([response_types{i_response} '_no_size_change'])(i_mu) = ...
                pombe_compute_response ( demand_vector(i_mu) , pars , response_types{i_response} , 'no' ) ;
        end
    end
end
...
for i_demand = 1:length(demand_types)
    ...
    for i_response = 1:length(response_types)
        ...
        writetable ( result_table , ['../output-data/pombe-demand-' demand_types{i_demand} ...
...
subplot ( 2 , 3 , 4 ) ; 
legend ( { 'no size change' } , 'FontSize' , 15 , 'Location' , 'NorthWest' ) ;
set ( gcf , 'Position' , [0 0 1600 1000] ) ; set ( gcf , 'Color' , 'w' ) ;
export_fig ( gcf , '../plots/pombe_Q_P_R_demands_km_kp_responses.pdf' ) ; close ;
