...
% load param sets
input_pars = readtable ( [ '../input-data/kp_scaling_with_mu/set_1.csv' ] ) ;

% prepare plots
plot_folder = [ '../plots/kp_scaling_with_mu/set_1/' ] ;
mkdir ( plot_folder ) ;

% make a subplot of alpha trajs
for i_sim = 1:size(input_pars,1)
    lineage_trajs = readtable ( [ '../output-data/kp_scaling_with_mu/set_1/parset-' num2str(i_sim) '_lineage-trajs.csv' ] ) ;
