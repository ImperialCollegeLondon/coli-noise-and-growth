...
% load param sets
input_pars = readtable ( [ '../input-data/no_change_with_mu/set_1.csv' ] ) ;

% prepare plots
plot_folder = [ '../plots/no_change_with_mu/set_1/' ] ;
mkdir ( plot_folder ) ;

% make a subplot of alpha trajs
for i_sim = 1:size(input_pars,1)
    lineage_trajs = readtable ( [ '../output-data/no_change_with_mu/set_1/parset-' num2str(i_sim) '_lineage-trajs.csv' ] ) ;
