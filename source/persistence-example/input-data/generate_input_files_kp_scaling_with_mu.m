...
km_slope_size = ones ( num_sets , 1) .* ( EM_birth(i_ref) * rm(i_ref) / V_birth(i_ref) ) ;
kp = ones ( num_sets , 1) .* ( rm(i_ref) * det.sigma / km_slope_size(i_ref) ) .* alpha_max ;
...
mkdir ( 'kp_scaling_with_mu' ) ;
writetable ( param_table , [ 'kp_scaling_with_mu/set_1.csv' ] ) ;
