...
% paramter that varies: mu_max
mu_max_vec = [ 1.75 2.0 2.25 ] ;
km_vec = 0.279 .* mu_max_vec ./ 2 ;
...
for i=1:length(mu_max_vec)
    ...
    plot ( [0 , mu_max_vec(end)] , [1 1] .* km_vec(i) ) ; hold on ;
    h = plot ( mu_max_vec(i) , km_vec(i) , 'ro' , 'MarkerFaceColor' , 'r' , 'MarkerSize' , 15 ) ;
    ylim ( [0.8 * min(km_vec) , 1.2 * max(km_vec)] ) ; xlim ( minmax ( mu_max_vec ) ) ;
    xlabel ( '\mu_{cell} (dblg/hr)' ) ; 
    if (i==1) ; ylabel ( 'km_{cell}' ) ; end
    if (i==1) ; legend ( h , {'\mu_{max} (media)'} , 'Location' , 'NorthEast' , 'FontSize' , 15 ) ; end
...
set ( gcf , 'Color' , 'None' ) ;
export_fig ( gcf , '../output-data/model-1/media_variation_km_scaling_mu_max' ) ; close ;
